...
function parseChatLog(current_chat_log, myClientId, title) {
  return current_chat_log.map(log => {
    const { sender: clientId, message } = log || {}
    const isMe = myClientId === clientId
    const name = isMe ? 'you' : `${title} ${clientId.slice(-4)}`
